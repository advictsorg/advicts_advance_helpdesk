...
class PortalHelpdesk(CustomerPortal):
    ...
    def portal_my_tickets(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby='create_by', **kw):
        ...
        return request.render("advicts_advance_helpdesk.portal_my_tickets", values)
    ...
    def portal_my_ticket_detail(self, ticket_id, access_token=None, report_type=None, message=False, download=False, **kw):
        ...
        if report_type in ('html', 'pdf', 'text'):
            return self._show_report(model=ticket_sudo, report_type=report_type, report_ref='advicts_advance_helpdesk.report_helpdeskticket', download=download)
        if ticket_sudo:
        ...
        return request.render("advicts_advance_helpdesk.portal_ticket_page", values)
...
#         if request.env.user.has_group('advicts_advance_helpdesk.helpdesk_group_manager'):
#             ticket_domain = []
...
#         return request.render("advicts_advance_helpdesk.portal_my_tickets",
#                               values)
...
#                 report_ref='advicts_advance_helpdesk.action_portal_report_sh_helpdesk_ticket',
#                 download=download)
#         if ticket_sudo:
#             if request.env.company.sh_receive_email_seeing_ticket:
#                 body = _('Ticket viewed by customer %s',
...
#         return request.render("advicts_advance_helpdesk.portal_ticket_page",
#                               values)
